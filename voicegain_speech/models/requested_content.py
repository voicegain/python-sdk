...
class RequestedContent(object):
    ...
    def full(self):
        ...
        Types of content to be returned when ASR is done with processing all audio.</br> Note 1: In order for [GET /asr/transcribe/{sessionId}/transcript](#operation/asrTranscribeGetTranscript) to return results - `words` needs to be selected. </br> Note 2: To suppress full result, specify empty array to replace the default.</br> Note 3: There is currently a bug (rcj-377) where if you specify only `transcript` then callback (if requested) will not contain any transcript. A workaround is to specify also `words`, however, this will result in the callback containing both transcript and the time-annotated words. </br> Note 4: There is currently a bug (rcj-380) where if you specify only `transcript` and do polling after polling.afterlife period expires you  will not get a transcript in response. A workaround is to specify also `words`, however, this will result in the polling response containing both transcript and the time-annotated words.   # noqa: E501
    ...
    def incremental(self):
        ...
        Types of incremental content to be returned from ASR while it is processing the audio: + **progress** - progress information + **words** - words with confidence and timing info + **word-tree** -- words with confidence and timing info plus word and parent ids - enables constructing a tree with recognition alternatives + **transcript** - just the text of transcription + **captions** - captions             There is no fixed default value but if `incermental` is **missing** then: + for **polling** with parameter **full=false** - only **progress** will be returned + if **callback** is requested then **no** incremental callback will be made (only full). BTW, for offline transcription no incremental callbacks are made at all.   # noqa: E501
