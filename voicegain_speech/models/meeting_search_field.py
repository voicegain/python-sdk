...
class MeetingSearchField(object):
    ...
    def possible_values(self):
        ...
        List of all possible values for the field. Present only if the number of values is limited. The values are expressed as strings. The field `type` determines the type and conversion from string. Dates should be in ISO format, e.g.: \"2019-09-18T17:57:43.110Z\".</br> Returned for: AUDIO_SRC, PHASE, METADATA (return keys), LANG, TAG (returns individual tag values),  KEYWORD (returns individual keyword values), SILENT (returns individual names), SPEAKER (returns individual names)   # noqa: E501
