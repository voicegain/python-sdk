...
class SettingsAsyncTranscription(object):
    ...
    def formatters(self):
        ...
        (optional) Configuration for text formatters.  Currently 7 types of formatters are available: + **basic** punctuation/capitalization formatter, + **enhanced** formatter (email, url, etc.) + **digits** formatter (incl. currency), + **profanity** masking formatter, + English US/GB **spelling** enforcer, + **redact** (PII redaction for CC, SSN, etc.) + **regex** (redaction using regex expressions) + **answer_redact** (redaction of the answer following a configured question) </br>  Note: all formatters, including **redact**, **regex**, and **answer_redact**, modify only the transcript and derived results — they never modify the submitted audio, the source Data Store object, nor the captured audio. To obtain PII-redacted audio use `audio.callback` (OFF-LINE transcription with a `fromUrl` source only) or the /sa/offline API with `piiRedaction.redactAudio`.</br>  More types of formatters with various options will be added in the future.</br> Some formatters, like e.g. \"basic\" are **default formatters**, which means that they can be considered as always included with their default parameter values. You need to explicitly specify them only if you need to override the default parameters or provide parameters which do nor have defaults. For example, to disable punctuation/capitalization you can provide  ``` [{\"type\" : \"basic\", \"parameters\" : {\"enabled\" : \"false\"}}] ``` Notice that all parameters values must be strings.</br> Enhanced formatter implies basic formatter turned on. Attempt to enable enhanced and disable basic will result in an error.   # noqa: E501
