...
class SettingsAsyncTranscription(object):
    ...
    def formatters(self):
        ...
        (optional) Configuration for text formatters.  Currently only digits formatter, basic punctuation/capitalization formatter, and english US/GB spelling enforcer are available. </br> More types of formatters with various options will be added in the future.</br> Some formatters, like e.g. \"basic\" are **default formatters**, which means that they can be considered as always included with their default parameter values. You need to explicitly specify them only if you need to override the default parameters or provide parameters which do nor have defaults. For example, to disable punctuation/capitalization you can provide  ``` [{\"type\" : \"basic\", \"parameters\" : {\"enabled\" : false}}] ```   # noqa: E501
