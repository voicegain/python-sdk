...
class SpeechAnalyticsResultDetail(object):
    ...
    def channels(self):
        ...
        Speech analytics results specific to each channel. There are 4 cases + **dual channel** input audio - `channelType=actual` - the `audioChannelSelector` will indicate the original audio channnels.  No diarization was done as each channel is assumed to have just one speaker. + **virtual dual** channel - `channelType=virtual` - the `audioChannelSelector` will indicate the virtual audio channnels. Diarization used was set to 2 speakers. + **single channel** audio with diarization - `channelType=diarized_speaker` - the `audioChannelSelector` will indicate which channel from the original audio was transcribed. Each element in `channels` list will correspond to 1 diarized speaker.  + **single channel** audio with speaker-timeline - `channelType=speaker-timeline` - the `audioChannelSelector` will indicate which channel from the original audio was transcribed. Each element in `channels` list will correspond to 1 speaker from the speaker timeline.  Some of the values are not available in two last cases, e.g., `gender`, `age`, `overtalk`.   `audioZones` and `decibels` can be obtained from transcript session via `transcribeSid`   # noqa: E501
