...
class PollReq(object):
    ...
    def __init__(self, afterlife=None, persist=300000, local_vars_configuration=None):  # noqa: E501
    ...
    def persist(self):
        ...
        Time (in msec) to retain the result data after processing has completed.  Default setting is 5 minutes.</br> For Cloud, the maximum persistence is 7 days.</br> For On-Prem/Edge deployments, the result data can be retained indefinitely if value is -1  (in this case you will need to explicitly DELETE the session)</br> As long as the `persist` has not expired the result data will be available for the poll request  and (if you are doing transcription) to  GET [/v1/asr/transcribe/{sessionId}/transcript](http://localhost:8082/internal-index.html#operation/asrTranscribeGetTranscript)  and GET [/v1/asr/transcribe/{sessionId}/data](#operation/asrTranscribeGetData)   # noqa: E501
