...
class AsrSettingsTranscriptionSA(object):
    ...
    def __init__(self, acoustic_model_non_real_time=None, acoustic_model_real_time=None, confidence_threshold=0.01, language_detection=None, languages=None, max_alternatives=1, optimized_for=None, sensitivity=0.5, speed_vs_accuracy=0.5, hints=None, diarization=None, content='words', local_vars_configuration=None):  # noqa: E501
    ...
    def acoustic_model_non_real_time(self):
        ...
        **Deprecated** &mdash; the [/sa](#tag/sa) API is now real-time only, so the non-real-time acoustic model has no effect here. For OFFLINE (batch) Speech Analytics use the [/sa/offline](#tag/sa-offline) API, which selects the offline acoustic model via its own settings.</br> Still accepted for backward compatibility but ignored by the real-time SA pipeline.   # noqa: E501
