...
class AsrSettingsTranscriptionCommon(object):
    ...
    def acoustic_model_non_real_time(self):
        ...
        (Optional) Name of an Acoustic Model that is to be used for offline and semi-real-time processing.  In the Cloud, the currently available offline models are: - **VoiceGain-omega** - the default model - best suited to general transcription tasks, - **VoiceGain-omega-x** - model trained on call-center calls  Only available for offline: - **whisper:medium**, also may use **whisper** as a shortcut for \"whisper:medium\" - **whisper:small**,  - **whisper:base**    If model name not provided, a suitable default will be used.</br> In some Edge deployments, may include version number separated by `:` , for example, **VoiceGain-BD_en-us:11**   # noqa: E501
