...
class CallMarker(object):
    ...
    def type(self):
        ...
        Type of the marker.  + `CC_ANSWER` - call-center answer - e.g. moment where a call transferred from voicebot to call-center has been answered (and typically put in a queue) + `CC_AGENT_ANSWER` - moment where call-center agent answered the call (typically picked it up from the queue) + `WARM_TRANSFER_START` - moment when a warm transfer process started + `WARM_TRANSFER_SUCCESS` - moment when a warm transfer was completed successfully + `WARM_TRANSFER_ABANDON` - moment when a warm transfer was abandoned (e.g. because the other side was unavailable and refused to pick up the call) + `HOLD` - moment when the call was put on hold + `UNHOLD` - moment when the call was taken off hold + `VM_START` - moment when a voicemail was started + `VM_END` - moment when a voicemail was ended   # noqa: E501
