...
class AsrCallbackApi(object):
    ...
    def asr_transcribe_callback(self, **kwargs):  # noqa: E501
        ...
        Needs to be implemented by the user of the API on a URL of your choice.</br> Receive speech transcription callback with results.</br> Request payload will contain either the intermediate or the final content of the transcription of the provided audio. The content of the callback payload is controlled using the sessions[].content parameters (incremental or full)  in the [/asr/transcribe/async](#operation/asrTranscribeAsyncPost) request. </br> Note that rmsData and vadData are not returned in the current implementation.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
