...
class SaApi(object):
    ...
    def sa_websocket_payload(self, **kwargs):  # noqa: E501
        """Websocket Payload for SA  # noqa: E501

        **This is not an HTTP API Spec.**</br> This is a workaround way to specify the payload format of websocket messages sent from Real-Time Speech Analytics. </br> See [POST /sa](#operation/saPost) on how to start new Real-Time Speech Analytics session.</br> </br> A single session will generate a stream of websocket messages with the results of transcription and speech analytics.</br> Notes about the payload:</br>   - only one of `word` or `wordEdit` may be present in the same websocket message    # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.sa_websocket_payload(async_req=True)
        >>> result = thread.get()
        ...
        :param _request_timeout: timeout setting for this request. If one
                                 ...
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        :return: SpeechAnalyticsWebsocketPayload
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        return self.sa_websocket_payload_with_http_info(**kwargs)  # noqa: E501

    def sa_websocket_payload_with_http_info(self, **kwargs):  # noqa: E501
        """Websocket Payload for SA  # noqa: E501

        **This is not an HTTP API Spec.**</br> This is a workaround way to specify the payload format of websocket messages sent from Real-Time Speech Analytics. </br> See [POST /sa](#operation/saPost) on how to start new Real-Time Speech Analytics session.</br> </br> A single session will generate a stream of websocket messages with the results of transcription and speech analytics.</br> Notes about the payload:</br>   - only one of `word` or `wordEdit` may be present in the same websocket message    # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.sa_websocket_payload_with_http_info(async_req=True)
        >>> result = thread.get()
        ...
        :param _request_timeout: timeout setting for this request. If one
                                 ...
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        :return: tuple(SpeechAnalyticsWebsocketPayload, status_code(int), headers(HTTPHeaderDict))
                 If the method is called asynchronously,
                 returns the request thread.
        """
        ...
        for key, val in six.iteritems(local_var_params['kwargs']):
            if key not in all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method sa_websocket_payload" % key
                )
            local_var_params[key] = val
        del local_var_params['kwargs']
        ...
        return self.api_client.call_api(
            ...
            post_params=form_params,
            files=local_var_files,
            response_type='SpeechAnalyticsWebsocketPayload',  # noqa: E501
            auth_settings=auth_settings,
            async_req=local_var_params.get('async_req'),
            _return_http_data_only=local_var_params.get('_return_http_data_only'),  # noqa: E501
            _preload_content=local_var_params.get('_preload_content', True),
