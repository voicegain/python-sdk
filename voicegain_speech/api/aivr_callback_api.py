...
class AivrCallbackApi(object):
    ...
    def aivr_callback_delete(self, csid, seq, **kwargs):  # noqa: E501
        ...
        Specification of a callback method that Voicegain Platform will use to communicate **the end** of a Telephony Bot Session to the User's dialog control logic.</br> **Note 1:** the url provided here is not relevant. User can implement this method on any url (consistent with corresponding PUT and DELETE methods).<br> **Note 2:** it is possible to configure AIVR app to send the `csid` either in the path (as in example here), in the query, or in the body of the callback request.  Request definition specifies what data will be sent from Voicegain to the User system.</br> Response definition specifies the data that may be sent from the User system to Voicegain.     # noqa: E501
    ...
    def aivr_callback_existing(self, csid, seq, **kwargs):  # noqa: E501
        ...
        Specification of a callback method that Voicegain Platform will use to communicate events from an existing Telephony Bot session to the User's dialog control logic.</br> **Note 1:** the url provided here is not relevant. User can implement this method on any url (consistent with corresponding PUT and DELETE methods).<br> **Note 2:** it is possible to configure Phone App to send the `csid` either in the path (as in example here), or in the query, or in the body of the callback request.  **Request** definition specifies the data that will be sent from Voicegain Telephony Bot Session to the User dialog system.</br>  **Response** definition specifies the data that may be sent from the User dialog system to the Voicegain Telephony Bot Session Engine.  The Response contains instructions for Voicegain Telephony Bot Session Engine (including TTS and ASR) in response to the data received in the Callback Request.</br> Possible actions that may be requested from Voicegain are: + prompt + question + disconnect + transfer  Only one action may be specified per Response.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
    ...
    def aivr_callback_new(self, **kwargs):  # noqa: E501
        ...
        Specification of a callback method that invoked by the Voicegain Platform to indicate a start of a new Telephony Bot session to the User's dialog control logic.</br> Note: the url provided here is not relevant. User can implement this method on any url (consistent with corresponding PUT and DELETE methods).  **Request** definition specifies the data that will be sent from Voicegain Telephony Bot Session to the User dialog system.</br>  **Response** definition specifies the data that may be sent from the User dialog system to the Voicegain Telephony Bot Session Engine.  The Response contains instructions for Voicegain Telephony Bot Session Engine (including TTS and ASR) in response to the data received in the Callback request.</br> Possible actions that may be requested from Voicegain are: + prompt + question + disconnect + transfer  Only one action may be specified per Response.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
    ...
    def aivr_callback_ws_new(self, **kwargs):  # noqa: E501
        """New Bot Session WS  # noqa: E501

        Specification of a callback method that invoked by the Voicegain Platform to indicate a start of a new Telephony Bot session to the User's dialog control logic.</br> This is the websocket session version which means that all subsequent communications will be done via websocket.</br> Note: the url provided here is not relevant. User can implement this method on any url (consistent with corresponding PUT and DELETE methods).   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.aivr_callback_ws_new(async_req=True)
        >>> result = thread.get()
        ...
        :param _request_timeout: timeout setting for this request. If one
                                 ...
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        :return: AIVRNewSessionWsResponse
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        return self.aivr_callback_ws_new_with_http_info(**kwargs)  # noqa: E501

    def aivr_callback_ws_new_with_http_info(self, **kwargs):  # noqa: E501
        """New Bot Session WS  # noqa: E501

        Specification of a callback method that invoked by the Voicegain Platform to indicate a start of a new Telephony Bot session to the User's dialog control logic.</br> This is the websocket session version which means that all subsequent communications will be done via websocket.</br> Note: the url provided here is not relevant. User can implement this method on any url (consistent with corresponding PUT and DELETE methods).   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.aivr_callback_ws_new_with_http_info(async_req=True)
        >>> result = thread.get()
        ...
        :param _request_timeout: timeout setting for this request. If one
                                 ...
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
        :return: tuple(AIVRNewSessionWsResponse, status_code(int), headers(HTTPHeaderDict))
                 If the method is called asynchronously,
                 returns the request thread.
        """
        ...
        all_params = ['aivr_new_session_ws']  # noqa: E501
        all_params.append('async_req')
        all_params.append('_return_http_data_only')
        all_params.append('_preload_content')
        all_params.append('_request_timeout')
        ...
        for key, val in six.iteritems(local_var_params['kwargs']):
            if key not in all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method aivr_callback_ws_new" % key
                )
            local_var_params[key] = val
        del local_var_params['kwargs']
        ...
        body_params = None
        if 'aivr_new_session_ws' in local_var_params:
            body_params = local_var_params['aivr_new_session_ws']
        # HTTP header `Accept`
        header_params['Accept'] = self.api_client.select_header_accept(
            ['application/json'])  # noqa: E501
        ...
        return self.api_client.call_api(
            '/customer/aivr-callback/ws', 'POST',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='AIVRNewSessionWsResponse',  # noqa: E501
            auth_settings=auth_settings,
            async_req=local_var_params.get('async_req'),
            _return_http_data_only=local_var_params.get('_return_http_data_only'),  # noqa: E501
            _preload_content=local_var_params.get('_preload_content', True),
