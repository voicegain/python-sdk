...
class TranscribeApi(object):
    ...
    def asr_transcribe_async_put(self, session_id, **kwargs):  # noqa: E501
        ...
        **Modify existing Transcription session**</br> This method should be invoked using the url returned in `sessions[].sessionUrl` field in the response to [POST /asr/transcribe/async](#operation/asrTranscribeAsyncPost) </br>  This command may be used to **pause** or **mute** the audio. (Note that you cannot mix mute and pause, i.e., you cannot pause if session is muted not mute when session is paused.)</br> </br> Using **pause** is simple: you issue the pause action:start command and about the same time you stop sending the audio. When you want to end the pause you re-start sending the audio and issue a pause action:stop command. The key factor that makes the pause work is stopping the audio stream, the pause command just instructs the recognizer to not terminate the recognition due to no audio being sent.</br> </br> **Mute** is more complicated. In order to use it you have to keep track of the amount of audio data sent to Voicegain. + To **start** mute you have to stop sending audio and note the audio time in milliseconds when you stopped.  Then you issue mute action:start command which requires the audio stop time in milliseconds in the `atTime` field.</br> + To **finish** mute you have to restart the audio.  You have to calculate the mute duration and then send mute action:stop command and set `atTime` to a value which is a sum of the audio time at mute start plus mute duration.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
