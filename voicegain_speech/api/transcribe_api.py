...
class TranscribeApi(object):
    ...
    def asr_transcribe_async_put(self, session_id, **kwargs):  # noqa: E501
        ...
        Modify existing Transcription session.</br> This command may also be used to pause or mute the audio. (Note that you cannot mix mute and pause, i.e. you cannot pause if session is muted not mute when session is paused.)</br> </br> Pause is simple: you issue the pause start command and about the same time stop sending the audio. When you want to end the pause you start sending the audio and issue pause stop command. The key factor that makes the pause work is stopping the audio stream, the pause command just instructs the recognizer to not stop recognition due to no audio being sent.</br> </br> Mute is more complicated. In order to use it you have to keep track of the amount of audio data sent to Voicegain. To start mute you have to stop sending audio and note the audio time in milliseconds when you stopped.  Then you issue mute start command which requires the audio stop time in milliseconds.</br> To finish mute you have to restart the audio.  You have to calculate the mute duration and then send mute stop command and pass the time which is a sum of the audio time (same as sent at mute start) plus mute duration.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
